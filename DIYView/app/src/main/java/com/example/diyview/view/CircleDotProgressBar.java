...
/**
* 分4部分：外围点的绘制，文字的绘制，按钮的绘制，点击事件的判断
* */

public class CircleDotProgressBar extends View {
    private static final int SHOW_MODE_ALL = 2;
    private static final int SHOW_MODE_PERCENT = 1;
    private static final int SHOW_MODE_NULL = 0;
    private static final int UNIT_TEXT_ALIGN_MODE_DEFAULT = 0;
    private static final int UNIT_TEXT_ALIGN_MODE_CN = 1;
    private static final int UNIT_TEXT_ALIGN_MODE_EN = 2;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (showMode != SHOW_MODE_NULL) {
            percentTextSize = ta.getDimension(R.styleable.CircleDotProgressBar_percentTextSize, SizeUtil.dp2px(30, getContext()));
        ...
        if (showMode == SHOW_MODE_ALL) {
            ...
            buttonTextSize = ta.getDimension(R.styleable.CircleDotProgressBar_buttonTextSize, SizeUtil.dp2px(15, getContext()));
            ...
            buttonTopOffset = ta.getDimension(R.styleable.CircleDotProgressBar_buttonTopOffset, SizeUtil.dp2px(15, getContext()));
            if (buttonText == null) {
        ...
        // 其他准备工作
        mSin_1 = (float) Math.sin(Math.toRadians(1)); // 求sin(1°)。角度需转换成弧度    概念：弧长等于半径的弧，其所对的圆心角为1弧度   所以 360°角=2π弧度
        mPaint = new Paint();
...
//            mPaint.setTypeface(Typeface.DEFAULT_BOLD); // 粗体
            ...
            Paint.FontMetrics fm = mPaint.getFontMetrics();//通过文字的Metrix拿到ascent和descent计算
...
//          float baseline = centerY + (fm.descent - fm.ascent) / 2 - fm.descent;
            float baseline = (getHeight() - fm.ascent + fm.descent) / 2 + fm.descent;//上下两公式都可
...
//            mPaint.setTypeface(Typeface.DEFAULT_BOLD); // 粗体
    ...
    interface ClickListener {
        void onClick(CircleDotProgressBar view);
    }
